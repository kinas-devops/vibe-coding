...
def simulate_episode(agent, code, training=True):
    ...
    for step in range(max_steps):
        ...
        # Reward shaping
        reward = 0.2 * fb[0] + 0.1 * fb[1]  # partial credit
        if fb[0] == len(code):
            reward = 1.0
        if guess in [g for g, _ in history]:
            reward -= 0.25  # discourage repetition
...
def train_agent(episodes=3000, difficulty="easy", update_target_every=10):
    ...
    plt.plot(smoothed)
    plt.title(f"Training Progress ({difficulty.capitalize()} Difficulty)")
    plt.xlabel("Episode")
...
if __name__ == "__main__":
    train_agent(episodes=3000, difficulty="easy")